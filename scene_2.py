...
CLEVR_OBJECTS = ("cube", "cylinder","cone")
...
def compute_bboxes(segmentation, asset_list):
  for k, asset in enumerate(asset_list, start=1):
    ...
    for t in range(segmentation.shape[0]):
      ...
      if idxs.size > 0:
        ...
        y_min = int(idxs[0].min())
        x_min = int(idxs[1].min())
        height = int((idxs[0].max() - y_min + 1))
        width = int((idxs[1].max() - x_min + 1))
        asset.metadata["bboxes"].append((x_min, y_min, width, height))
        asset.metadata["bbox_frames"].append(t)
      if idxs_int.size > 0:
        x_coords = idxs[1]  # x 坐标
        y_coords = idxs[0]  # y 坐标
        mask_coordinates = np.empty((len(x_coords) + len(y_coords),), dtype=np.int32)  # 使用 int32
        mask_coordinates[0::2] = x_coords
        mask_coordinates[1::2] = y_coords
        asset.metadata["mask"].append(mask_coordinates)
...
compute_bboxes(data_stack["segmentation"], visible_foreground_assets)
# 生成物体masks
